...
char   message[256] = {0};           ///< Memory for the string that is passed from userspace//trying to add message as an export symbol
...
static struct device* LKM_input_Device = NULL; ///< The device-driver device struct pointer
 ...
 */
static int __init LKM_input_device_driver_init(void){
    printk(KERN_INFO "LKM_input_device_driver: Initializing the LKM_input_device_driver LKM\n");
    
    // Try to dynamically allocate a major number for the device -- more difficult but worth it
    majorNumber = register_chrdev(0, DEVICE_NAME, &fops);
    if (majorNumber<0){
        printk(KERN_ALERT "LKM_input_device_driver failed to register a major number\n");
        return majorNumber;
    }
    printk(KERN_INFO "LKM_input_device_driver: registered correctly with major number %d\n", majorNumber);
    
    // Register the device class
    LKM_input_class = class_create(THIS_MODULE, CLASS_NAME);
    if (IS_ERR(LKM_input_class)){                // Check for error and clean up if there is
        unregister_chrdev(majorNumber, DEVICE_NAME);
        printk(KERN_ALERT "Failed to register device class\n");
        return PTR_ERR(LKM_input_class);          // Correct way to return an error on a pointer
    }
    printk(KERN_INFO "LKM_input_device_driver: device class registered correctly\n");
    
    // Register the device driver
    LKM_input_Device = device_create(LKM_input_class, NULL, MKDEV(majorNumber, 0), NULL, DEVICE_NAME);
    if (IS_ERR(LKM_input_Device)){               // Clean up if there is an error
        class_destroy(LKM_input_class);           // Repeated code but the alternative is goto statements
        unregister_chrdev(majorNumber, DEVICE_NAME);
...
static void __exit LKM_input_device_driver_exit(void){
    ...
    printk(KERN_INFO "LKM_input_device_driver: Goodbye from the LKM!\n");
}
...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
    ...
    if (error_count==0){            // if true then have success
        printk(KERN_INFO "LKM_input_device_driver: Sent %d characters to the user\n", size_of_message);
        return (size_of_message=0);  // clear the position to the start and return 0
    }
    else {
        printk(KERN_INFO "LKM_input_device_driver: Failed to send %d characters to the user\n", error_count);
 ...
 *  listed above)
 */
module_init(LKM_input_device_driver_init);
module_exit(LKM_input_device_driver_exit);
